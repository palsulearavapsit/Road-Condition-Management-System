 ...
 * 
 * Usage: cd crackx-app && node test-supabase.js
 */
...
async function testConnection() {
    try {
        ...
        if (bucketsError) {
            ...
            console.log(`   Found ${buckets?.length || 0} bucket(s):`, buckets?.map(b => b.name).join(', ') || 'none');

            const reportImagesBucket = buckets?.find(b => b.name === 'report-images');
            if (!reportImagesBucket) {
                console.error('❌ Storage bucket "report-images" not found via API');
                console.log('\n💡 The bucket exists in the database (we verified via SQL)');
                console.log('   but the API can\'t list it. This might be a permissions issue.');
                console.log('   The app should still work - try submitting a report!');
                console.log('\n⚠️  Continuing with other tests...');
            } else {
